...
export class TrainingSubCategoriesService {
  ...
  async findOne(id: string,  header: CustomHeaders) {
    ...
    const findSubCategory: any =
      await TrainingSubCategoriesEntity.findOne({
        where: {
          id: id,
        },
        relations: {
  ...
  async create(body: CreateTrainingSubCategoryDto) {
    ...
    const findSubCategory = await TrainingSubCategoriesEntity.findOneBy({title: body.title}) 
    if (findSubCategory) {
      throw new HttpException(
        'Alrady add sub Category',
        HttpStatus.FOUND,
      );
    }
  ...
  async update(
    id: string,
    body: UpdateTrainingSubCategory,
  ) {
    ...
    const findCategory  =  await TrainingCategoriesEntity.findOneBy({id: body.category_id})
